...
serveStill(bool wantBmp)
{
  ...
  auto client = server.client(); // Store the client in a variable
  server.send(200, wantBmp ? "image/bmp" : "image/jpeg");
  frame->writeTo(client); // Use the variable here
}
...
serveMjpeg()
{
  ...
  auto client = server.client();                      // Store the client in a variable
  int nFrames = esp32cam::Camera.streamMjpeg(client); // Use the variable here
